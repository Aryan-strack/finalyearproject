...
// @route   GET /api/vendors/profile/id/:profileId
// @desc    Get vendor profile by profile ID
// @access  Public
router.get('/profile/id/:profileId', async (req, res) => {
  try {
    const vendorProfile = await VendorProfile.findById(req.params.profileId)
      .populate('userId', 'name email');
...
// @access  Private (Vendor only)
// @route   GET /api/vendors/:userId/stats/public
// @desc    Get public vendor stats (products, followers, sales)
// @access  Public
router.get('/:userId/stats/public', async (req, res) => {
  try {
    ...
    // Get real-time stats
    const [productCount, followerCount, totalSales] = await Promise.all([
      Product.countDocuments({ vendor: req.params.userId }),
      Follow.countDocuments({ vendorId: req.params.userId }),
      Order.aggregate([
        { $match: { 'items.vendorId': req.params.userId, status: 'completed' } },
        { $unwind: '$items' },
        { $match: { 'items.vendorId': req.params.userId } },
    ...
    res.json({
      ...
      stats: {
        ...
        averageRating: vendorProfile.stats?.averageRating || 0,
        reviewCount: vendorProfile.stats?.reviewCount || 0
      }
    });

  } catch (error) {
    console.error('Error fetching public vendor stats:', error);
    res.status(500).json({
      success: false,

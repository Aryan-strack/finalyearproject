...
// @access  Private (Admin only)
router.get('/revenue-chart', [auth, isAdmin], async (req, res) => {
  try {
    // Get monthly revenue for the last 12 months (from delivered orders)
...
// @route   GET /api/admin/user-growth-chart
// @desc    Get monthly user growth data for chart
// @access  Private (Admin only)
router.get('/user-growth-chart', [auth, isAdmin], async (req, res) => {
  try {
    ...
    // Group by month and separate vendors and customers
    const groupedData = {}
    monthlyUsers.forEach(item => {
      ...
      if (item._id.role === 'vendor') {
        groupedData[monthKey].vendors = item.count
      } else if (item._id.role === 'customer') {
        groupedData[monthKey].customers = item.count
      }
    })
...
// @route   GET /api/admin/order-status-chart
// @desc    Get order status distribution for chart
// @access  Private (Admin only)
router.get('/order-status-chart', [auth, isAdmin], async (req, res) => {
  try {
    ...
    const chartData = orderStatus.map(item => ({
...
// @access  Private (Admin only)
router.get('/analytics-summary', [auth, isAdmin], async (req, res) => {
  try {
    const now = new Date()
    ...
    const revenue30DaysResult = await Order.aggregate([
      { $match: { status: 'delivered', createdAt: { $gte: thirtyDaysAgo } } },
      { $group: { _id: null, total: { $sum: '$total' } } }
    ])
    const revenue30Days = revenue30DaysResult.length > 0 ? revenue30DaysResult[0].total : 0

    // Revenue last 7 days
    const revenue7DaysResult = await Order.aggregate([
      { $match: { status: 'delivered', createdAt: { $gte: sevenDaysAgo } } },
      { $group: { _id: null, total: { $sum: '$total' } } }
    ])
    const revenue7Days = revenue7DaysResult.length > 0 ? revenue7DaysResult[0].total : 0
    ...
    const users30Days = await User.countDocuments({
      createdAt: { $gte: thirtyDaysAgo }
    })

    // New users last 7 days
    const users7Days = await User.countDocuments({
      createdAt: { $gte: sevenDaysAgo }
    })

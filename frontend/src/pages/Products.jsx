...
import { FiSearch, FiFilter, FiGrid, FiList, FiHeart, FiShoppingCart, FiStar, FiChevronDown, FiChevronUp, FiX, FiCamera } from 'react-icons/fi'
...
const Products = () => {
  ...
  // Handle incoming AI products from location state (e.g., from Navbar search)
  useEffect(() => {
    if (location.state?.aiProducts) {
      handleVisualSearchResults(location.state.aiProducts, location.state.analysis)
      // Clear location state to prevent re-applying on every render
  ...
  const handleVisualSearchResults = (aiProducts, analysis) => {
    // Transform AI products to match expected format if needed
    const transformedRes = aiProducts.map(product => ({
      _id: product._id,
      name: product.name,
      price: product.price,
      ...
      rating: product.rating || 0,
      reviewCount: product.reviewCount || 0,
      vendorName: product.vendor?.shopName || product.vendorName || 'Unknown Vendor',
      category: product.category || 'General',
      image: product.images?.[0] || 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=500&h=500&fit=crop',
      description: product.description || 'Beautiful handmade product',
      tags: product.tags || ['handmade', 'artisan'],
    ...
    setFilteredProducts(transformedRes)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-gray-100 to-slate-100 py-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="flex-1 w-full lg:w-auto">
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 transform -translate-y-1/2 p-2 bg-gray-100 hover:bg-blue-100 text-gray-500 hover:text-blue-600 rounded-lg transition-all duration-200"
        ...
        {/* AI Analysis Result Chip */}
        {aiAnalysis && (
          <div className="flex items-center space-x-2 mb-6 p-4 bg-blue-50/80 backdrop-blur-sm border border-blue-200 rounded-2xl animate-in slide-in-from-left duration-500">
            <div className="p-2 bg-blue-600 text-white rounded-lg">
              <FiCamera className="w-4 h-4" />
            </div>
            <div className="flex-1">
              {/* <p className="text-sm text-blue-800">
                AI identified this as: <span className="font-bold underline">"{aiAnalysis.description}"</span>
              </p> */}
              <div className="flex flex-wrap gap-2 mt-2">
                {aiAnalysis.keywords.slice(0, 5).map(tag => (
                  <span key={tag} className="text-[10px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-medium">#{tag}</span>
                ))}
              </div>
            </div>
            <button
              onClick={() => { setAiAnalysis(null); setFilteredProducts(products); }}
